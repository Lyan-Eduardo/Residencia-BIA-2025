"""
Treino de modelos Random Forest para:
...
# Hiperparâmetros básicos do Random Forest
N_ESTIMATORS = 200
MAX_DEPTH = None   # pode limitar ex: 20 se quiser controlar overfitting
...
# ----------------------------------------------------------------------
# 5. Normalização para o modelo de tempo (não é obrigatória pro RF,
#    mas deixa o pipeline mais consistente para outros modelos)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 6. Treino dos modelos Random Forest
#    - Classificador: próxima atividade
#    - Regressor: tempo até próxima atividade
# ----------------------------------------------------------------------

print("Treinando RandomForestClassifier para próxima atividade...")
...
rf_act.fit(X_train, y_train_act)

print("Treinando RandomForestRegressor para tempo até próxima atividade...")
...
rf_time.fit(X_train_scaled, y_train_time)
...
model_act_path = os.path.join(OUTPUT_MODELS_DIR, "rf_next_activity.pkl")
model_time_path = os.path.join(OUTPUT_MODELS_DIR, "rf_time_to_next.pkl")
scaler_path = os.path.join(OUTPUT_MODELS_DIR, "rf_time_scaler.pkl")

dump(rf_act, model_act_path)
dump(rf_time, model_time_path)
...
results_csv_path = os.path.join(
    OUTPUT_RESULTS_DIR,
    f"rf_next_activity_and_time_{EVENTLOG}"
)
