...
with open('output_files/folds/fold1.csv', 'w', newline='') as csvfile:
    ...
    for row, timeseq in zip(fold1, fold1_t):
        spamwriter.writerow([f"{s}#{t}" for s, t in zip(row, timeseq)])
    ...
    for t_idx, char in enumerate(sentence):
        ...
        for c in chars:
            if c == char:  # this will encode present events to the right places
                X[i, t_idx + leftpad, char_indices[c]] = 1
        X[i, t_idx + leftpad, len(chars)] = t_idx + 1
        X[i, t_idx + leftpad, len(chars) + 1] = sentence_t[t_idx] / divisor
        X[i, t_idx + leftpad, len(chars) + 2] = sentence_t2[t_idx] / divisor2
        X[i, t_idx + leftpad, len(chars) + 3] = sentence_t3[t_idx] / 86400
        X[i, t_idx + leftpad, len(chars) + 4] = sentence_t4[t_idx] / 7
...
act_output = Dense(len(target_chars), activation='softmax',
...
model_checkpoint = ModelCheckpoint(
    'output_files/models/model_{epoch:02d}-{val_loss:.2f}.h5',
...
model.fit(
    X, {'act_output': y_a, 'time_output': y_t},
    validation_split=0.2, verbose=2,
    callbacks=[early_stopping, model_checkpoint, lr_reducer],
    batch_size=maxlen, epochs=500
)